...
interface Servidor {
  ...
  urlAgenteRecursos?: string; // Endpoint remoto personalizado para agente JSON
...
2. **`ServicioMonitoreo`**: Gestiona verificaciones asíncronas en paralelo (`Promise.all()`).  
   - **Lógica de Priorización:** Evalúa primeramente el HTTPS. Si una máquina no expone puertos TCP internos pero la suma de **todas sus URLs asignadas responden `disponible`**, el motor descarta el fallo TCP y asigna dinámicamente el estado como `ok`.
   - **Monitoreo de Recursos Real (Agente Remoto):** Evalúa el uso de CPU, RAM y Disco consultando un Agente ligero remoto. Ahora soporta parametrización individual de la ruta del agente (`urlAgenteRecursos`) por servidor para permitir integraciones en infraestructuras restrictivas (ej. IIS .NET handler). En caso de defecto o ausencia de URL, asume puerto 9000. Superar los umbrales transita el servidor a alerta.
3. **Verificadores**:
