...
2. **`ServicioMonitoreo`**: Gestiona verificaciones asíncronas en paralelo (`Promise.all()`).  
   ...
   - **Monitoreo de Recursos Real (Agente Remoto):** Se integra lógica complementaria para consultar métricas de hardware mediante un Agente remoto ligero habilitado en cada servidor (`/metrics`). En caso de que el Agente esté inalcanzable, la interfaz señala el error sin falsear alertas de capacidad. Si estas mediciones superan sus umbrales definibles por el usuario, el servicio de monitoreo lo detecta y lo notifica.
3. **Verificadores**:
